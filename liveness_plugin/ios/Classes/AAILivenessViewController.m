...
- (void)viewDidLoad
{
    ...
    if (self.detectionActions != NULL) {
        wrapView.detectionActions = self.detectionActions;
    }
    ...
    [_wrapView startAuthWithCompletionBlk:^(NSError * _Nonnull error) {
        ...
        if (strongSelf) {
            ...
            if (error) {
                ...
                NSDictionary *userInfo = @{ @"code": @"FAILED", @"message": error.localizedDescription, @"isSuccess": @(NO)};
                [[NSNotificationCenter defaultCenter] postNotificationName:@"kAAILivenessDetectComplete" object:nil userInfo:userInfo];
                /*
...
- (void)onDetectionFailed:(AAIDetectionResult)detectionResult forDetectionType:(AAIDetectionType)detectionType
{
    ...
    if (key) {
        ...
        NSDictionary *userInfo = @{ @"code": @"FAILED", @"message": state, @"isSuccess": @(NO)};
        [[NSNotificationCenter defaultCenter] postNotificationName:@"kAAILivenessDetectComplete" object:nil userInfo:userInfo];
        /*
...
- (void)onDetectionComplete:(AAILivenessResult *)resultInfo
{
    ...
    NSString *base64Str = [UIImageJPEGRepresentation(bestImg, 1) base64EncodedStringWithOptions:NSDataBase64Encoding64CharacterLineLength];
    NSDictionary *userInfo = @{
        @"base64Image": base64Str,
        @"livenessId": livenessId,
        @"isSuccess": @(YES),
        @"message": @"SUCCESS",
        @"transactionId": resultInfo.transactionId
    };
    [[NSNotificationCenter defaultCenter] postNotificationName:@"kAAILivenessDetectComplete" object:nil userInfo:userInfo];
    /*
